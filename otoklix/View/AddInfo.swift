...
final class AddInfo: UIView {
    private let infoLabel = UILabel()
    private let button = UIButton()
    
    var buttonPressed: (() -> Void)?
    var successAddInfo: ((Post)-> Void)?
    ...
    func set(vm: PostAddViewModel) {
        vm.state.listen { [weak self] state in
            ...
            case .success(let post):
                ...
                self.button.setTitle("Post", for: .normal)
                self.infoLabel.text = "Success Posting"
                UIView.animate(withDuration: 2) {
                    self.infoLabel.alpha = 0
                }
                self.successAddInfo?(post)
            case .error:
                self.button.isEnabled = true
                self.button.setTitle("Post", for: .normal)
                self.infoLabel.text = "Fail Posting"
                UIView.animate(withDuration: 2) {
                    self.infoLabel.alpha = 0
                }
            }
